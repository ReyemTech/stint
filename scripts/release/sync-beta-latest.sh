...
# Find the versioned DMG (e.g. Stint-0.2.0-beta.1.dmg). The beta cask
# uses a per-version URL (same shape as stable) so we mirror it by name
# rather than renaming to a fixed "Stint-Beta-latest.dmg".
DMG_SRC=""
...
# Recreate as a fresh prerelease pointing at the current commit, mirroring
# the same assets as the versioned tag. The DMG keeps its `Stint-<version>.dmg`
# filename so the beta cask's livecheck regex matches.
